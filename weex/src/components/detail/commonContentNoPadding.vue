<template>
  <div class="detail_wrapper">
    ...
    :key_index="(index)" :key_len="(getcontentList.length-1)"
    :style="[
    {'border-bottom-width':(getcontentList.length-1) == index ? 'unset' : '1px'},
    {'border-bottom-style':(getcontentList.length-1) == index ? 'unset' : 'solid'},
    {'border-bottom-color':(getcontentList.length-1) == index ? 'unset' : 'rgba(51, 51, 51, 0.08)'},
    ]" v-for="(list, index) in getcontentList" :key="index">
