<template>
  <div class="container">
    <div class="content">
      ...
      <div class="cellList">
        <div class="cellItem">
          <text class="itemLeft">默认账户：</text>
          <div class="itemRightClick" @click="checkDefaultAccount">
            <image class="check-img" :src="checkSrc" v-if="defaultAccount=='DFLT'"></image>
            <image class="check-img" :src="uncheckSrc" v-if="defaultAccount=='NDFT'"></image>
          </div>
        </div>
      </div>
    </div>
    <div class="bottom" v-if="isshowButton=='NDFT'">
...
export default {
  ...
  data() {
    return {
      checkSrc: "./imgs/mobileTransferRegister/open.png",
      ...
      popupData: {
        ...
        ActChType: "3",
        resultPath: "/MTChangeResultDefault"
      },
      orderType: [],
      paramsPopup: {},
      isTrue: false,
    ...
    ActMsgGet() {
      this.goJson("/refacmob/ActMsgGet.do", {}, response => {
        ...
        if (res.State == "1") {
          ...
          this.defaultAccount = res.DftAccType;
          this.isshowButton = res.DftAccType;
        }
      });
    },
    checkDefaultAccount() {
      if (this.isshowButton == "DFLT") {
        return;
      }
      if (this.defaultAccount == "DFLT") {
        this.defaultAccount = "NDFT";
      } else {
        this.defaultAccount = "DFLT";
      }
    },
    judgeData() {
      if (this.defaultAccount == "NDFT") {
        this.weexAlert("请勾选默认账户");
      } else {
        this.toConfirm();
      }
    },
    toConfirm() {
      this.goJson("/refacmob/ActMsgChgConfirm.do", {}, response => {
        ...
        this.paramsPopup = {
          ...
          ActChType: "3",
          DftAccType: this.defaultAccount,
