<template>
  <div>
    ...
    <div class="newGroup">
      ...
      <div v-if="$store.state.isshowAdd">
        <div class="add" @click="addMembers2">
          <image :src="addImg" class="add_img" />
          <text class="add_title">添加组员</text>
        </div>
        <div
          class="graybg"
          :style="{ top: this.getPosiScrollerTop() + 250 + 'px' }"
        ></div>
      </div>
      <div v-if="!$store.state.isshowAdd">
        ...
        <list
          class="info"
          :style="{ top: this.getPosiScrollerTop() + 220 + 'px' }"
        >
          <cell
      ...
      <div class="section_bottom">
        <div :class="[newGroupName ? 'save' : 'gray']" @click="save">
          <text class="next_btn">保存</text>
        </div>
      </div>
    </div>
    <deleteNewGroupMeber></deleteNewGroupMeber>
  </div>
</template>
<script>
...
export default {
  components: { NavBar, WxcMask, deletegroup, deleteNewGroupMeber },
  data() {
    return {
  ...
  created() {
    ...
    this.newGroupName=this.$store.state.newGroupName;
  },
   computed: {
    newGroupNumber() {
      return this.$store.state.newGroup.length;
    },
  },
  methods: {
     addMembers2() {
      ...
      if (!this.newGroupName) {
        ...
        return;
      }else if(this.newGroupName.length>10){
