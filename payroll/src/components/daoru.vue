<template>
  <div>
    <wxc-mask
      ...
      :has-overlay="true"
      :show="$store.state.isDaoruCancel"
      :overlay-can-close="false"
    >
      <div class="daoru">
        <text class="daoru_text">确认取消导入?</text>
        <div class="daoru-footer">
          <text class="cancel" @click="cancel2">取消</text>
          <div class="daoru_xian"></div>
          <text class="comfirm" @click="comfirmExcel2">确定</text>
        </div>
      </div>
    </wxc-mask>
  </div>
...
export default {
  ...
  methods: {
    // 取消导入
    cancel2() {
      this.$store.state.isDaoruCancel = false;
    },
    // 确定
    comfirmExcel2() {
      this.$store.state.isDaoruCancel = false;
      // 删除excel表
       for(let i in this.$store.state.payrollEntry){
        for(let j in this.$store.state.excelList){
          if(this.$store.state.payrollEntry[i].PayeeAcNo==this.$store.state.excelList[j].PayeeAcNo){
