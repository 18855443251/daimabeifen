<template>
  <div class="editaccount">
     ...
     <nav-bar :title="title"></nav-bar>
       ...
       <div class="all"></div>
       <div class="netinfo" >
            <div class="netaddr"  @click="cliAddress">
            ...
            <div class="netaddr" @click="getnetqry">
      ...
      <div class="editalis">
            ...
            <input type="text" class="aliasinput" maxlength="11" v-model="Phone"  placeholder="请输入手机号码"/>
      </div>
      <!-- <div class="all"></div> -->
      <div class="editalis">
...
export default {
    ...
    data () {
      return {
        ...
        addressObj:"",
        ProvinceName:this.$store.state.bankCarsInfo.ProvinceName,
        seladdressname:this.$store.state.bankCarsInfo.CityName,
        CityCode:this.$store.state.bankCarsInfo.CityCode,
        ProvinceCode:"",
  ...
  mounted(){
    ...
    //本行 不显示网点     还有SysFlag=1也不显示 林志军说的
  ...
  computed:{
   ...
   methods: {
      getnetqry(){
            this.goJson('/eweb/PayeeBankDeptListQry.do',{
                  PayeeBankId:this.$store.state.bankCarsInfo.PayeeBank,
      ...
      okfinish(){
        // modal.alert({
        //   message:"城市"+this.CityCode
        // })
        // modal.alert({
        //   message:"银行"+this.$store.state.bankCarsInfo.PayeeBank
        // })
        // modal.alert({
        //   message:"支行"+this.deptid
        // })
          // if(this.aliasname == ''){
          //       // modal.alert({
          //       //     message:'别名不能为空'
          //       // })
          //       wjalert.alert('别名不能为空',"确定",data=>{
          //       })
          //     return
          //  }
            this.goJson('/eweb/PayeeBookManage.do',{
                  ...
                  PayeeAcNoOld:this.$store.state.bankCarsInfo.PayeeAcNo,
                  PayeeAcName:this.$store.state.bankCarsInfo.PayeeAcName,
              ...
              this.$store.state.bankCarsInfo.Phone=this.Phone;
              this.$store.state.bankCarsInfo.PayeeacAlias=this.aliasname;
              this.$store.state.bankCarsInfo.BankCode = this.deptid;
              this.$store.state.bankCarsInfo.CityCode = this.CityCode;
              this.$store.state.bankCarsInfo.ProvinceName = this.ProvinceName;
              this.$store.state.bankCarsInfo.CityName = this.seladdressname
              this.$store.state.bankCarsInfo.LName = this.dept;
        ...
        selectedaddress(event){
          if(event === ''){
            ...
            this.ProvinceName = event.split(' ')[0];
          ...
          let items = this.deptname;
            ...
            event => {
              if (event.result === "success") {
                ...
                this.dept = this.deptname[event.data];
